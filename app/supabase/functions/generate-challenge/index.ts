...
//
// Wraps the server-side AI generator (server/services/challengeGenerator.ts)
// for client-callable use. The Anthropic API key lives as a Supabase secret
...
//   6. Log to generation_log either way.
//
...
Deno.serve(async (req) => {
  ...
  // Avoids both a wasted Anthropic round-trip and a unique-index violation.
  ...
  // Try AI generation. On any failure, fall back to challenge_bank.
  try {
    ...
    if (!aiResp.ok) {
      throw new Error(`Anthropic ${aiResp.status}: ${await aiResp.text()}`);
    }
    ...
    const first = aiJson.content[0];
    const raw = first && first.type === 'text' && first.text ? first.text.trim() : '';
    const clean = raw.replace(/```json|```/g, '').trim();
    const parsed: unknown = JSON.parse(clean);
    assertValidResult(parsed);

    const inserted = await insertChallenge(supabase, userId, today, parsed.main);
    if (!inserted) {
      // Race: someone else (another device, retry) inserted in parallel.
      const fetched = await fetchExistingMain(supabase, userId, today);
      if (fetched) return json({ ok: true, challenge: fetched });
      return json({ ok: false, error: 'generic' }, 500);
    }

    const tokensUsed = aiJson.usage.input_tokens + aiJson.usage.output_tokens;
    await logGeneration(supabase, userId, today, tokensUsed, 'success');
    return json({ ok: true, challenge: inserted });
  } catch (err) {
    console.error('[generate-challenge] AI path failed, falling back:', err);
    const fallback = await pickFallback(supabase, userId);
    if (!fallback) {
      await logGeneration(supabase, userId, today, 0, 'error', errorMessage(err));
      return json({ ok: false, error: 'generic' }, 500);
    }
    ...
    await logGeneration(supabase, userId, today, 0, 'fallback', errorMessage(err));
    return json({ ok: true, challenge: inserted });
  }
});
