...
- Fixed invisible buttons on native (Welcome "Започни сега" CTA, Login "Влез" primary button, Continue with Apple / Google, Settings rows incl. Sign out / Delete account, Survey options, Restore screen, ErrorBoundary retry, and the various modal save/cancel buttons inside Settings). Root cause: function-form `Pressable` `style` (`style={({ pressed }) => ({ ... })}`) was silently no-op'd on native in this NativeWind v4 + `babel-preset-expo({ jsxImportSource: 'nativewind' })` + RN 0.81 setup — the returned style object never reached the native view, so backgroundColor / borderWidth / flexDirection / etc. all disappeared and buttons rendered as bare text on the page bg. Web rendered correctly because react-native-web translates `style` via a different path. Fix: converted all 23 function-form `Pressable` styles to static-form `style={{ ... }}` (the form that BottomTabBar uses and that works on native). Trade-off: the `pressed`-state visual feedback (opacity/scale dip on tap) is gone — the native ripple on Android still provides feedback, but iOS taps now have no in-button visual response. Acceptable for MVP; can be restored later via `onPressIn`/`onPressOut` + local state or an Animated wrapper. Files touched: `WelcomeScreen.tsx`, `AuthScreen.tsx`, `SettingsScreen.tsx`, `SurveyScreen.tsx`, `RestoreScreen.tsx`, `ErrorBoundary.tsx`, `HomeScreen.tsx`.
- Custom fonts (Plus Jakarta Sans, Inter) loaded via `expo-font` + `@expo-google-fonts/plus-jakarta-sans` + `@expo-google-fonts/inter`. `useFonts` is called in `app/_layout.tsx` and the entire tree is gated on `fontsLoaded || fontError` to avoid FOUT — splash is a single `View` painted in the app bg color (`#FAFAF7`). A font-load error falls through to render with system fallback (logged to Sentry) rather than stranding the user on the splash. Five variants are loaded: `PlusJakartaSans_{600SemiBold,700Bold}` + `Inter_{400Regular,500Medium,600SemiBold}` — the keys match the runtime fontFamily strings already referenced across every screen and `tailwind.config.js`. Safe-area insets respected on the three main screens (`HomeScreen`, `HistoryScreen`, `ProfileScreen`) via `useSafeAreaInsets()` → `paddingTop: insets.top + 40` (replaces the previous hardcoded `60`, which clipped content under the notch on modern iPhones).
...
- Challenza brand foundation — app name + chibi cel-shaded phoenix mascot artwork locked in. `<Mascot variant size testID>` (`src/components/Mascot.tsx`) is a thin `Image` wrapper over 8 typed variants (`hero | inviting | curious | celebrating | triumphant | resting | sad | silhouette`); only `hero` has real art today (`assets/mascot/hero.png`), the other seven fall back to hero via a per-variant `require()` map with a `__DEV__`-only `console.warn` fired once per missing variant (gated by a module-level `Set` so strict-mode double-invocation doesn't double-log). `accessibilityLabel` pulls from `mascot.${variant}` i18n keys (8 leaf keys added to both `en.json` and `bg.json`, structures parallel). Brand palette exposed two ways: `BRAND` const + `BrandColor` union in `src/theme/colors.ts` (`phoenixRed #FA4C03`, `sunsetOrange #FA9D06`, `crestYellow #FDDE31`, `emberDeep #5B0807`, `warmCream #FAFAF7`, `linen #FFF6E5`, `mutedClay #8B7E76`), and matching Tailwind tokens in kebab-case (`bg-phoenix-red`, `text-ember-deep`, etc.) — additive only, no existing color keys renamed. `app.json` `icon` / `splash.image` / `android.adaptiveIcon.foregroundImage` repointed to `./assets/icons/app-icon.png` / `./assets/splash.png` / `./assets/icons/adaptive-icon.png` (cream `#FAFAF7` background + `contain` resize already matched the spec). Screen wiring deliberately narrow: `WelcomeScreen` renders `<Mascot variant="hero" size={180} />` at the top of the centered hero block; `AuthScreen` renders `<Mascot variant="hero" size={88} />` in its `AuthHeader` (login + register share this header); `HomeScreen` renders `<Mascot variant="hero" size={160} />` strictly inside the `!mainChallenge` pre-generation branch (above `ChallengeMeHero`) — skeleton/error/active-card/completed branches all untouched, so the mascot only appears in the pre-tap empty state. The legacy `LogoGlyph` feather SVG was removed from both `WelcomeScreen` and `AuthScreen` (the only two screens that defined a private copy); the matching `logoStroke` theme key was removed from each file's `THEMES` block at the same time. No new dependencies; uses bare `react-native` `Image`.
- Mascot idle animation landed — subtle continuous breathing/sway via `react-native-reanimated@~4.1.1` (already in `package.json`, no install needed). New `state?: 'static' | 'idle'` prop on `Mascot.tsx` defaults to `'static'`, which renders byte-identical to the prior implementation; `state='idle'` wraps the Image in `Animated.View` and drives two shared values from a single `useEffect([state, reducedMotion])`: `scale` 1.0↔1.035 over 2×1200ms (2400ms period) and `rotate` ±1.5deg over 2×1400ms (2800ms period), both `Easing.inOut(Easing.quad)`, both `withRepeat(withSequence(...), -1, true)`. The slight out-of-phase periods prevent a robotic lockstep feel. `useReducedMotion()` from Reanimated short-circuits the effect: when true (or when `state !== 'idle'`) the effect calls `cancelAnimation` on both shared values and resets them to base — the worklet never starts. All animation lives on the UI thread via shared values; no JS-thread timers, `setInterval`, or `useEffect` loops driving values. `WelcomeScreen`, `AuthScreen` header (login + register), and `HomeScreen` pre-generation empty state all pass `state="idle"`; no other call sites added (specifically not on the loading skeleton, the active challenge card, or the completed state). Babel plugin verified — note that the project is on Reanimated 4, which moved the worklet plugin out into a separate package, so `babel.config.js` carries `'react-native-worklets/plugin'` rather than the Reanimated-3-era `'react-native-reanimated/plugin'`; functionally equivalent, already last (and only) entry in the plugins array. Deferred: a celebration animation triggered on challenge completion needs a UI design decision about where the mascot appears in the completed state before it can be implemented.
...
- `Pressable` `style` prop must be static-form `style={{ ... }}` across the app, **never** function-form `style={({ pressed }) => ({ ... })}`. The function-form is silently dropped on native (RN 0.81 / Expo SDK 54 / NativeWind v4 + `jsxImportSource: 'nativewind'`) — the returned object never reaches the native view, so backgroundColor / borderWidth / flexDirection / etc. disappear. We standardized on static-form everywhere; if you reintroduce function-form, the touched component will render invisible/unstyled on the device but look fine in the web preview. Trade-off: no `pressed`-state visual feedback (opacity/scale dip). To re-add later, use `onPressIn`/`onPressOut` + a local `useState`, or wrap children in an `Animated.View` driven by a `useSharedValue`. Root-cause analysis (NativeWind babel pipeline) tracked as a follow-up rather than a blocker.
...
│   │   ├── Mascot.tsx              # Phoenix mascot wrapper — 8 typed variants, hero fallback, i18n a11y
...
│   ├── theme/
│   │   └── colors.ts               # BRAND palette constants + BrandColor union (matches Tailwind tokens)
│   ├── store/                      # Zustand
