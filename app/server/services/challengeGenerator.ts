...
export async function generateForUser(userId: string): Promise<void> {
  ...
  // Idempotency: a main row for today already exists → exit silently. The
  // DB unique index `challenges_one_main_per_day` is the hard guard; this
  // check just avoids the wasted Anthropic round-trip.
...
// --- Batch generation (currently unused) ---
//
// MVP shipped without a cron — generation is user-initiated via the
// "Challenge me!" button (Edge Function `generate-challenge`). This batch
// helper is retained as scaffolding for a future scheduled regeneration job
// (e.g. proactive top-up at 02:00, or notifications for users who haven't
// generated by mid-morning). No caller exercises it today.
...
async function insertFallbackChallenges(userId: string, date: string): Promise<void> {
  // Mirror the selection logic that migration 007's `seed_first_day_challenges`
  // RPC used to encode: prefer rows whose category matches a user goal, else
  // fall back to a random easy row in the user's language. Single row only —
  // bonus is out of MVP scope.
  ...
  const rawGoals = (profile as { goals?: string[] } | null)?.goals ?? [];
  const VALID: ReadonlyArray<Category> = [
    'health',
    'mental',
    'productivity',
    'social',
  ...
  // Prefer rows whose category is in goalCategories; if none match, any easy
  // row in the language is acceptable.
